...
void main() {
	...
	vec2 brush_uv = clamp(gl_GlobalInvocationID.xy, VEC_ZERO, params.brush_size);

	// Calculate the position in the current image to sample from
	vec2 brush_half_size = params.brush_size / 2.0;
	vec2 uv = clamp(brush_uv + params.brush_position - brush_half_size, VEC_ZERO, params.texture_size);
